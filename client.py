...
if __name__ == "__main__":
    ...
    # (2) '(max(2, 3) + 3)' = 6
    expr1 = add_b(max_f(2, 3), 3) # (2)

    # (3) '3 + ((4 * 2) / ((1 - 5) ** (2 ** 3)))' = 3.0001220703125
    expr2 = add_b(3, div_b(mul_b(4, 2), pow_b(sub_b(1, 5), pow_b(2, 3)))) # (3)

    # (4) '((1 + 2) ** (3 * 4)) / (5 * 6)' = 17714.7
    expr3 = div_b(pow_b(add_b(1, 2), mul_b(3, 4)), mul_b(5, 6)) # (4)

    # (5) '-(-((1 + (2 + 3)) ** -(4 + 5)))' = 9.92290301275212e-08
    expr4 = neg_u(neg_u(pow_b(add_b(1, add_b(2, 3)), neg_u(add_b(4, 5))))) # (5)

    # (6) 'max(2, (3 * 4), log(e), (6 * 7), (9 / 8))' = 42
    expr5 = max_f(2, mul_b(3, 4), log_f(e_c), mul_b(6, 7), div_b(9, 8)) # (6)
    # * Change in end (1)
    ...
    print("1: Expression 0 - (sin(max(...)))")
    print("2: Expression 5 - max(2, 3*4, log(e)...)")
    ...
    print("\n client socket close,end of Process ")